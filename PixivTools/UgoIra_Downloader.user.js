/*globals zip, GIF, globalInitData, createImageBitmap */
// ==UserScript==
// @name        UgoIra Downloader
// @description Download image archive for animated illustrations
// @version     2
// @author      noccu
// @namespace   pixiv_fuckery
// @match       *://www.pixiv.net/*/illust/*
// @run-at      document-start
// @noframes
// @grant       none
// @require     https://raw.githubusercontent.com/jnordberg/gif.js/master/dist/gif.js
...
var iMeta = null;
var iData = Object.values(globalInitData.preload.illust)[0];

(function init() {
    var load = new Promise(function (r, x) {
        let int, to;
        ...
        to = setTimeout(function() {
            clearTimers();
            x("UgoIra injection timeout.");
        }, 10000);
    });

    load.then( function(container) {
        //let iData = Object.values(globalInitData.preload.illust)[0];
        if (iData.illustType == ILLUST_TYPES.UGOIRA) {
            let dlArch = createButton("ZIP Frames", downloadZIP);
            let dlGif = createButton("Create GIF", downloadGIF);
            dlGif.download = `${iData.illustId} - ${iData.illustTitle}.gif`;
            container.appendChild(dlArch);
            container.appendChild(dlGif);
        }
    }
             );
})();
...
function createButton(text, onclick) {
    ...
    //b.setAttribute("class", "add-bookmark _button");
    b.text = text;
    //b.href = link;
...
function getUgoIraMeta(id) {
    return fetch(`https://www.pixiv.net/ajax/illust/${id}/ugoira_meta`,
...
function downloadZIP(button) {
    function updButton() {
        button.href = iMeta.originalSrc;
        button.onclick = null;
        button.click();
    }

    if (!iMeta) {
        getUgoIraMeta(iData.illustId)
        .then(data => iMeta = data.body)
        .then(updButton);
    }
    else {
        updButton();
...
function downloadGIF(button) {
    function process () {
        button.text = "Loading...";
//        button.style.color = "grey";
//        fetch(iData.originalSrc)
//        .then(resp => resp.arrayBuffer())
//        .then(buf => loadZip(buf));

        zip.useWebWorkers = false;
        zip.createReader(new zip.HttpReader(iMeta.originalSrc), r => createGIF(r, button));
    }
...
function createGIF (reader, btn) {
    var sBlob = new Blob([gifWorker],{type: "text/javascript"});
    var sWorker = URL.createObjectURL(sBlob);
    var gif = new GIF({workerScript: sWorker,
                       quality: 2,
                       workers: 4,
...
//        if (canvas.width != bitmap.width) { canvas.width = bitmap.width;}
        ...
        gif.addFrame(ctx.getImageData(0, 0, canvas.width, canvas.height), {delay: iMeta.frames[n].delay});
        n++;
        return n;
    }
    ...
    function processData (blob) {
        ...
        createImageBitmap(blob)
            .then(addFrame)
            .then(function(n) {
            if (n >= iMeta.frames.length) {
                reader.close( function(){
                    console.log("Closed reader, rendering gif.");
                    gif.render();
                });
            }
            })
    ...
    //Assuming the zip reader/structure always has files in the right order.
...
//        let writer = new zip.BlobWriter();
        ...
        for (let entry of entries) {
            ...
            entry.getData(new zip.BlobWriter(iMeta.mime_type), processData);
        }
    });

    gif.on("progress", function(p) {
       btn.textContent = `Rendering... ${p*100}%`;
    });
    gif.on("finished", function(gifBlob) {
        URL.revokeObjectURL(sWorker);
        btn.onclick = null;
        btn.href = URL.createObjectURL(gifBlob);
