...
// ==UserScript==
// @name        TwitterTools (non Tampermonkey version)
...
dbgLog("global", window);


//I'm not sure if this is FF (and their CSP fails) or WebExtensions or whatever but shit is dumb.
//Tampermonkey is the only thing that works (on FF at least, haven't tried non-TM on Chrome but apparently it should work) without this shit.
window.setTimeout("(" + hack.toString() + ")()", 0);
window.addEventListener("freedonuts", eat);
function hack() {
    console.error("hack");// DEBUG
    //Hack into Twitter requests for data gathering.
    XMLHttpRequest.prototype.realSend = XMLHttpRequest.prototype.send;
    XMLHttpRequest.prototype.send = function (val) {
        this.addEventListener("load", function () {
            window.dispatchEvent( new CustomEvent("freedonuts", {detail: this}) );
...
function eat(evt) {
    ...
    let xhr = evt.detail;
    if (/videos\/tweet\/config\/\d+\.json/.test(xhr.responseURL)) {
        let data = JSON.parse(xhr.response).track;
...
function csAttachEvent(evtName, repeat) {                     
    window.setTimeout(`($(document).${repeat ? "on" : "one"}("${evtName}", ev => window.dispatchEvent( new CustomEvent("csEvent", {detail: {csName: "${evtName}", csEvent: ev}}) ) ); )()"`, 0);
}

function csEvtHandler (e) {
    switch (e.detail.csName) {
        case "uiPageChanged":
            processPage();
            break;
        case "uiHasInjectedTimelineItem":
            processTimelineItem(e.detail.csEvent);
            break;
        case "uiHasInjectedTimelineItem":
            processTimelineItem(e.detail.csEvent);
    }
}
window.addEventListener("csEvent", csEvtHandler);
//end bs
...
(function main() {
    ...
    new Promise((r, e) => {
            iv = setInterval(() => {
                if (window.$ || (unsafeWindow && unsafeWindow.$) || (window.wrappedJSObject && window.wrappedJSObject.$)) {
                    dbgLog("found jq", window, unsafeWindow);
                    r();
                }
            }, 500);
            to = setTimeout(() => {
                dbgLog("timeout", window, unsafeWindow);
                e();
            }, 15000);
        })
        .then(() => {
...
function processPage() {
    csAttachEvent("uiPageChanged", false); //Reactive itself upon changes.
...
function processTimeline(tl) {
    ...
    csAttachEvent("uiHasInjectedTimelineItem", true);
...
function addGifContent({data: {td,
                               c: container}
                       }) {
    var item,
