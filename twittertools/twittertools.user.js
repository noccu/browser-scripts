...
(function main() {
    ...
    new Promise((r, e) => { iv = setInterval(() => {if (window.$) r();}, 500);
...
function ttSetLocation() {
    switch (true) {
        case location.host == "twitter.com":
            TT.console = (function() { let i = document.createElement("iframe");
                                      i.style.display = "none";
                                      try {document.body.appendChild(i);}
                                      catch (e) { ttStatusUpdate(TT.statuses.LOG, "Couldn't add console.\n" + e);
...
function processStream(stream) {
    if (stream.ttProcessed) {return ttStatusUpdate(TT.statuses.ERROR, "Already processed this stream.");}
    ...
    if (td.content.present) {
        ...
        switch (td.content.type) {
            ...
            case "gif":
                ...
                $(td.content.eventElement).on("uiPlayableMediaReady", {td, c: dlAction.container}, addGifContent); //Not using .one() cause for some reason the event is fired twice and the first doesn't indicate the dom change.
                break;
            default:
...
function parseTweet(t) {
    ...
    var images = [],
        video = {thumb: "", url: "", player: null},
        contentCount = 0;

    if (hasContent) {
        ({type, element:cardEl} = parseTweetType(t));
...
function addVideoContent(td, container) {
    ...
    if (td.content.video.type == "mp4") {
        ...
        item.link.onclick = function (e) {
            ...
            createVideoURL(td)
                .then(blobUrl => {item.link.href = blobUrl;
...
function addGifContent({ data: {td, c:container} }) {
    var item,
...
function createDownloadAction() {
    ...
    return {base: action, container: cont};
}
...
function addDownloadLink(cont, dst, thumb) {
    ...
    return {'item': item, 'link': link, 'symbol': symbol};
}
...
function createVideoURL (td) {
    ...
    .then( d => {if (d.type != "master") throw "Expected master playlist at " + d.from;
                 if (d.streams.length < 1) throw "Found no media playlists in " + d.from;
                 return getPlaylistData(d.streams.pop().url);
                })
    .then( d => {if (d.type != "media") throw "Expected media playlist at " + d.from;
