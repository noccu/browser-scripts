...
//end bs
exportFunction(processPage, window.wrappedJSObject, {
  defineAs: "processPage"
});
...
(function main() {
    ...
    new Promise((r, e) => {
            iv = setInterval(() => {
                if (window.$ = window.$ || (unsafeWindow && unsafeWindow.$) || (window.wrappedJSObject && window.wrappedJSObject.$)) r();
            }, 500);
            setTimeout(e, 15000);
        })
        .then(() => {
...
function processPage() {
    $(document).one("uiPageChanged",  window.wrappedJSObject.processPage); //Reactive itself upon changes.
